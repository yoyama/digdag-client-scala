...
class AttemptRestTest extends FlatSpec with Matchers {
  ...
  "Valid json" should "parsed" in {
    ...
    attempts match {
      case Success(a) => {
        assert(a.size == 2)
        assert(a.head.sessionTime ==  OffsetDateTime.parse( "2019-05-02T15:12:38+00:00", SessionRest.dateTimeFormatter))
